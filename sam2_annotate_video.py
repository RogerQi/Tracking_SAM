...
with tempfile.TemporaryDirectory() as temp_folder:
    ...
    for frame_idx in sorted(video_segments.keys()):
        mask_frames.append(video_segments[frame_idx][OBJ_IDX][0].astype(np.uint8) * 255)
    
    # Write to video
    out_video_path = "/home/roger/annotate_video_out.mp4"
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(out_video_path, fourcc, 30, (mask_frames[0].shape[1], mask_frames[0].shape[0]))
